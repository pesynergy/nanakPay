...
class FundController extends Controller
{
            ...
            case "upiintent":
                ...
                switch ($api->code) {
                    ...
                    case 'dgpay':
                        ...
                        $url = 'https://pg.digycart.com/api/seamless/txnReq';
                        
                        $Data = [
                            'mid' => $mid,
                            'orderNo' => "NPSELF".date("ymdhis").$post->user_id,
                            'amount' => $post->amount,
                            'currency' => 'INR',
                            'txnReqType' => 'S',
                            'undefinedField1' => '',
                            ...
                            'respUrl' => '',
                            'optional1/upiType' => 'intent'
                        ];
                        // Create the $RequestSequence string by concatenating keys of $Data array
                        $RequestSequence = implode("|", array_keys($Data));
                        $hashVarsSeq = explode('|', $RequestSequence);
                        
                        // Initialize an empty string for building the hash string
                        $hash_string = '';
                        $i = 1;
                        ...
                        $encryptedData = $this->encrypt($hashString, $secretkey, $saltkey, $key_len);
    ...
    protected function encrypt($data, $key) {
        ...
        $key = $this->fixKey($key);
        $key = $this->derivateKey($key, $saltkey, 65536, 256); 
    ...
    protected function fixKey($key) {
        if (strlen($key) < $key_len) { //0 pad to len 35
            return str_pad($key, $key_len, "0"); 
        }
        if (strlen($key) > $key_len) { //truncate to 35 bytes
            return substr($key, 0, $key_len);
        }
        return $key;
    }
}
